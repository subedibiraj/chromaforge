...
\author{
    ...
    \small \href{https://github.com/birajsubedi/chromaforge}{github.com/birajsubedi/chromaforge} \\
...
All three models are trained on the COCO 2017 dataset and
evaluated on peak signal-to-noise ratio (PSNR), structural similarity (SSIM),
and the Hasler--S\"usstrunk colorfulness metric. Our findings confirm that while
the regression baselines achieve competitive or higher PSNR and SSIM scores, they
systematically produce desaturated, low-chrominance outputs. The cGAN formulation
yields comparable PSNR but substantially higher colorfulness ($\sim$32 vs.
$\sim$14--16 for the baselines), consistent with the adversarial objective's
bias toward perceptually plausible rather than pixel-accurate predictions. We
discuss the implications of this trade-off and identify conditions under which
each formulation is preferable.
...
(see Section~1.5 for a detailed review of related work)
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../figures/demo_grid.png}
\caption{Example colorization outputs. From left to right: input grayscale (L channel), ChromaForge cGAN colorization, and original ground truth.}
\label{fig:demo}
\end{figure}
...
\textbf{Learning-based colorization.}
The earliest fully automatic deep learning approach to colorization was proposed
by Cheng et al.~(2015), who used a four-layer neural network with hand-crafted
features. Zhang et al.~\cite{zhang2016colorization} reformulated colorization as
a classification problem over quantized AB color bins and introduced a
class-rebalancing loss to combat the dominance of low-saturation colors. Their
work established the LAB-space formulation used in this study.

\textbf{Conditional adversarial networks.}
Isola et al.~\cite{isola2017pix2pix} proposed the pix2pix framework, a general
purpose conditional GAN for image-to-image translation tasks including
colorization, segmentation, and edges-to-photos. Their U-Net generator and
70$\times$70 PatchGAN discriminator form the architectural basis of our cGAN
model. The combination of adversarial and L1 losses was shown to produce sharper,
more colorful outputs than L1 alone.

\textbf{Instance-aware and semantic approaches.}
Su et al.~(2020) introduced InstColorization, which applies separate
colorization networks to detected object instances and the full image,
fusing the results to handle multi-object scenes. Semantic conditioning via
pre-trained features (e.g., VGG, CLIP~\cite{radford2021clip}) has been explored
to resolve color ambiguity by providing high-level context.

\textbf{Diffusion-based colorization.}
More recently, diffusion probabilistic models have been applied to
colorization. Saharia et al.~\cite{saharia2022palette} demonstrated that
Palette, an image-to-image diffusion model, can produce diverse, high-quality
colorizations by sampling from the learned posterior distribution. This
addresses the one-to-many nature of colorization more principally than GANs,
though at significantly higher computational cost.

Our work does not propose a new method. Instead, we present a careful
implementation and controlled comparison within the pix2pix framework,
isolating the contributions of architecture (CNN vs.\ U-Net) and loss function
(L1 vs.\ adversarial + L1) under matched experimental conditions.
...
split with a seeded random shuffle (seed = 42) to ensure reproducibility across
runs. Images are resized to $256\times256$ using Lanczos resampling.
...
The cGAN model (Model~3) was trained on the full COCO 2017 training set for
100 epochs. The two baseline models (Models~1 and~2) were trained on a
20,000-image random subset for 20 epochs due to computational constraints.
We acknowledge this asymmetry as a limitation (see Section~7); the baseline
results therefore represent a lower bound on what these architectures could
achieve with matched compute. Despite this disadvantage, the baselines still
achieve competitive PSNR and SSIM, which strengthens the conclusion that
colorfulness differences are primarily attributable to the loss function.

\begin{center}
\begin{tabular}{llll}
\toprule
\textbf{Hyperparameter} & \textbf{CNN Baseline} & \textbf{U-Net + L1} & \textbf{cGAN} \\
\midrule
Training images  & 20,000     & 20,000     & 118,287 \\
Epochs           & 20         & 20         & 100 \\
Batch size       & 16         & 8          & 8 \\
Optimizer        & \multicolumn{3}{c}{Adam ($\beta_1=0.5$, $\beta_2=0.999$)} \\
Learning rate    & $2{\times}10^{-4}$  & $2{\times}10^{-4}$  & $2{\times}10^{-4}$ \\
LR schedule      & constant   & constant   & constant 50 ep., then linear decay \\
$\lambda_{L1}$   & ---        & ---        & 100 \\
GAN real label   & ---        & ---        & 0.9 (label smoothing) \\
Weight init      & \multicolumn{3}{c}{$\mathcal{N}(0, 0.02)$ for Conv; $\mathcal{N}(1, 0.02)$ for BN} \\
Augmentation     & H-flip     & H-flip     & H-flip + brightness jitter $\in [0.8, 1.2]$ \\
Hardware         & RTX 3060   & RTX 3060   & RTX 3060 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{table}[H]
\centering
\caption{Validation set results on COCO 2017. PSNR and SSIM are computed between
predicted and ground-truth colorizations. Colorfulness is a reference-free
perceptual metric measured on the predicted images only.
$\uparrow$ = higher is better. Best value in each column is \textbf{bolded}.
Note: baselines were trained on a 20k-image subset for 20 epochs; the cGAN
was trained on the full 118k dataset for 100 epochs (see Section~4.2).}
\label{tab:results}
\begin{tabular}{lccc}
\toprule
...
U-Net + L1                & \textbf{23.74} & \textbf{0.914} & 16.1 \\
U-Net + cGAN (ours)       & 23.67 & 0.891 & \textbf{31.8} \\
\bottomrule
\end{tabular}
\end{table}

% Include training curves figure if available
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figures/training_curves.pdf}
\caption{Training dynamics across all three models. (Top-left) Generator and
discriminator losses for the cGAN. (Top-right) PSNR vs.\ epoch.
(Bottom-left) SSIM vs.\ epoch. (Bottom-right) Colorfulness vs.\ epoch.
The cGAN converges to dramatically higher colorfulness while maintaining
competitive PSNR and SSIM.}
\label{fig:training_curves}
\end{figure}

\subsection{Discussion}

Several observations follow from Table~\ref{tab:results} and
Figure~\ref{fig:training_curves}.

\textbf{U-Net skip connections improve PSNR and SSIM.} Comparing the CNN
baseline to the U-Net + L1 model isolates the architectural contribution: skip
connections improve best PSNR by 0.50~dB (23.24 $\to$ 23.74) and SSIM by 0.005
(0.909 $\to$ 0.914). This confirms that spatial detail preserved through skip
paths is beneficial for chrominance prediction, consistent with the original
U-Net findings~\cite{ronneberger2015unet}.

\textbf{The adversarial objective dramatically increases colorfulness at modest
PSNR cost.} The cGAN achieves a colorfulness score of 31.8 — nearly double the
U-Net + L1 (16.1) and more than double the CNN baseline (13.7). This comes at a
modest PSNR reduction of 0.07~dB relative to the U-Net + L1 model (23.67 vs.\
23.74). For reference, natural photographs in COCO typically score 25--40 on the
colorfulness metric, placing the cGAN's predictions within the natural range
while the L1-only models produce visibly desaturated outputs.

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{../figures/colorfulness_comparison.pdf}
\caption{Colorfulness distribution (last 5 epochs)}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{../figures/metric_summary.pdf}
\caption{Final metric comparison}
\end{subfigure}
\caption{Comparison of regression vs.\ adversarial objectives. The cGAN dramatically increases colorfulness without significantly harming PSNR or SSIM.}
\label{fig:metrics}
\end{figure}

\textbf{PSNR and SSIM favor desaturation.} An important observation is that the
CNN baseline achieves a high SSIM (0.909) despite producing the least colorful
outputs. This is because pixel-level metrics reward the ``safe'' strategy of
predicting near-grey values. When the model is uncertain between red and blue,
predicting grey minimizes MSE. Colorfulness scores expose this failure mode:
PSNR alone would suggest the baseline is nearly as good as the cGAN.

\textbf{Colorfulness is a necessary companion to PSNR.} These results
reinforce the argument made by Isola et al.~\cite{isola2017pix2pix} and
Zhang et al.~\cite{zhang2016colorization}: pixel-level metrics are insufficient
for evaluating generative image tasks. The colorfulness gap between models
(13.7 vs.\ 31.8) is immediately apparent to human observers but invisible
in the PSNR figures.
...
\textbf{Unmatched training compute for baselines.} The CNN baseline and U-Net +
L1 models were trained on a 20,000-image subset for 20 epochs, while the cGAN
was trained on the full 118,287-image dataset for 100 epochs. This asymmetry
means the baseline PSNR/SSIM values represent a lower bound; with matched compute,
the baselines would likely achieve higher scores. However, we note that even
under this disadvantage, the baselines achieve \emph{competitive} PSNR and SSIM,
which strengthens the conclusion that colorfulness differences are primarily
attributable to the loss function, not the training data volume. A controlled
replication with matched compute is planned.

\textbf{Missing perceptual metrics.} We report PSNR, SSIM, and colorfulness but
omit Fr\'echet Inception Distance (FID) and Learned Perceptual Image Patch
Similarity (LPIPS), which are standard in the generative modeling literature.
Future work should include these metrics, evaluated on a held-out test set
(COCO 2017 val, 5,000 images) rather than validation batches sampled during
training.
...
codebase, training scripts, and model weights are available at
\url{https://github.com/birajsubedi/chromaforge}.
